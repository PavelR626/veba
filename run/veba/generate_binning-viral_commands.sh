...
do
	...
	params="-f ${fasta} -b ${bam} -n ${id} -o ${output_directory} -p=${n_threads_per_job} -m 1500 --veba_database ${veba_database}"
        cmd="/usr/bin/time -v veba --module ${module} --params \"${params}\""
	echo "echo ${job_name}" >> ${commands_file}
