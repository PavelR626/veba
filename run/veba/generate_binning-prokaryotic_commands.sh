...
do
        ...
        # Marine on GPU
        # params="-f ${fasta} -b ${bam}  -n ${id} -o ${output_directory} -p=${n_threads_per_job} -I 10 -m 1500 -a metabat2,metadecoder,semibin2-global,semibin2-ocean,vamb --semibin2_engine gpu --vamb_engine gpu --veba_database ${veba_database}"
        # Soil on CPU
        params="-f ${fasta} -b ${bam}  -n ${id} -o ${output_directory} -p=${n_threads_per_job} -I 10 -m 1500 -a metabat2,metadecoder,semibin2-global,semibin2-soil,vamb --semibin2_engine cpu --vamb_engine cpu --veba_database ${veba_database}"
